...
public class Sight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < colliders.Length; i++) {
            ...
            // Debug.Log(characterName + " detected a something");
            ...
            if (angleToCollider < angle) {
                if (!Physics.Linecast(transform.position, collider.bounds.center, out RaycastHit hit, obstaclesLayers)) {
                    ...
                    // Debug.Log(characterName + " detects " + collider.gameObject.name);
                    detectedObject = collider;
                    // if (collider.gameObject.CompareTag("Paw")) {
                    //     Debug.Log(characterName + " detects paw");
                    //     detectedObject = collider;
                    // }
                    // if (collider.gameObject.CompareTag("Body")) {
                    //     Debug.Log(characterName + " detects body");
                    //     detectedObject = collider;
                    // }
                }
                else {
